...
# 업데이트 체크
class XXMI_TOOLBOX_OT_check_update(Operator):
    bl_idname = "xxmi_toolbox.check_update"
    bl_label = "업데이트 체크"
    bl_description = "최신 버전이 있는지 확인합니다"

    def execute(self, context):
        try:
            url = "https://api.github.com/repos/DPN-dpn/XXMI-Toolbox/releases/latest"
            with urllib.request.urlopen(url, timeout=5) as response:
                data = json.loads(response.read().decode())
                latest_version = data.get("tag_name", "").lstrip("v")
                context.scene["xxmi_toolbox_latest_version"] = latest_version
                # 현재 버전 가져오기
                from ... import bl_info

                current_version = ".".join(map(str, bl_info["version"]))
                context.scene["xxmi_toolbox_current_version"] = current_version
                context.scene["xxmi_toolbox_show_restart"] = False
                if latest_version and latest_version != current_version:
                    context.scene["xxmi_toolbox_update_available"] = True
                    self.report(
                        {"INFO"}, f"업데이트 가능: {current_version} → {latest_version}"
                    )
                else:
                    context.scene["xxmi_toolbox_update_available"] = False
...
# 업데이트 실행
class XXMI_TOOLBOX_OT_do_update(Operator):
    bl_idname = "xxmi_toolbox.do_update"
    bl_label = "업데이트"
    ...
    def execute(self, context):
        ...
        try:
            ...
            context.scene["xxmi_toolbox_show_restart"] = True
            _redraw_ui_regions(context)
            return {"FINISHED"}
...
# GitHub 이동
class XXMI_TOOLBOX_OT_open_github(Operator):
    bl_idname = "xxmi_toolbox.open_github"
    bl_label = "GitHub"
    ...
    def execute(self, context):
        ...
        webbrowser.open("https://github.com/DPN-dpn/XXMI-Toolbox")
        self.report({"INFO"}, "XXMI-Toolbox GitHub 페이지를 엽니다.")
        return {"FINISHED"}


classes = (
    XXMI_TOOLBOX_OT_check_update,
    XXMI_TOOLBOX_OT_do_update,
    XXMI_TOOLBOX_OT_open_github,
)
